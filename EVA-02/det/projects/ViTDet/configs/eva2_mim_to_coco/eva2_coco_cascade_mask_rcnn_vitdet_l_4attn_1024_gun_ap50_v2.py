"""EVA-02 Large + ARMED: Gun AP50 v2 — targeted improvements for Gun/Weapon.

Builds on the one-shot-ap50 config but applies focused changes to break
the Gun AP50 plateau (~22) without hurting Armed (~45) or Unarmed (~41):
 ...
 4. Multi-scale anchors per FPN level for better small-object recall
 5. Stronger repeat-factor threshold (0.05) for Gun oversampling
 6. Low-quality cascade matching in stage 0 → every GT gun gets a proposal
 7. Slightly more aggressive scale augmentation (min_scale=0.5, max_scale=2.5)
 8. Lower soft-NMS IoU threshold (0.2) so overlapping Gun+Armed survive
 9. Extended schedule to 100k with cosine-warm milestones
"""
...
# ---------------------------------------------------------------------------
# 2) Stronger class-imbalance mitigation: raise threshold from 0.01 → 0.05.
...
# ---------------------------------------------------------------------------
# 3) Multi-scale anchors per FPN level for denser small-object coverage.
#    2 sizes per level (vs 1 in v1) → 6 anchors per cell (2 × 3 aspect ratios).
# ---------------------------------------------------------------------------
model.proposal_generator.anchor_generator.sizes = [
    [8, 16],      # p2: very small guns (~8-16px)
    [16, 32],     # p3
    [32, 64],     # p4
    [64, 128],    # p5
    [128, 256],   # p6
...
#    gradient signal to adjust Gun-specific features.
# ---------------------------------------------------------------------------
optimizer.lr = 5e-6

# ---------------------------------------------------------------------------
# 10) Schedule: 100k iters with milestones at 80k and 90k.
#     Longer training + higher LR = chance to converge better on Gun.
#     Warmup 2000 iters (longer warmup for stability with higher LR).
# ---------------------------------------------------------------------------
