"""EVA-02 Large + ARMED one-shot AP50 experiment.

L-variant analog of `eva2_coco_cascade_mask_rcnn_vitdet_b_4attn_1024_one_shot_ap50.py`.
Inherits from the L base armed config and applies the EXACT same nine
overrides used in the B one-shot run, in the same order, with the same
values, so the only difference between the two runs is backbone capacity
(B = 86M vs L = 304M).

NOTE: SPD in SimpleFeaturePyramid (vit.py) and focal loss in
FastRCNNOutputLayers (fast_rcnn.py) are SHARED-CODE changes that apply to
this config automatically; nothing extra to wire.
"""
...
# 1) Small-object focused augmentation policy (same as B one-shot).
...
# 3) Smaller RPN anchors (image-space) for tiny objects (e.g. guns) at p2 to p6.
#    Keeps one size per level so num_anchors=3 (x aspect ratios) is unchanged.
